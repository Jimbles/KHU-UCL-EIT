function [ DC_offset_data ] = khu_check_DCoffset( varargin )
%CHECK_DCOFFSET check DC offset for all channels and all frequencies by
%default
...
if isempty(find(strcmp('DCOffset',foldernames),1)) == 1
...
thresholdvalue=25; %value which constitues a bad channel
thresholdrange=10; %max variation between channels before warning
...
for f=freqsv %loop through every freq selected
    ...
    DCdata{freq_counter}=dlmread(filepath);
    Chn(freq_counter)=length(DCdata{freq_counter}); %find number of channels
end
...
%stick all of the DC offset data in a matrix for easier calling
...
for N=Chn_use
    ...
    %they are ok
    temp_cal=DCdata{N}; %get DC calibration data
    ...
    %ylimit is threshold rounded to nearest 10
    ymax=ceil(thresholdvalue/10)*10;
    
    %if only positive then dont show negative values
    if length(temp_cal(temp_cal > 0 )) ~= length(temp_cal)
        ymins=-ymax;
    else
        ymins=0;
    end
    ...
    %guide lines for threshold value
    line([0  length(temp_cal)+1],[thresholdvalue thresholdvalue],'color','r','linestyle','-','linewidth',4)
    line([0  length(temp_cal)+1],[-thresholdvalue -thresholdvalue],'color','r','linestyle','-','linewidth',4)
    
    text(1,thresholdvalue-1,'FUZZY LOGIC LINE OF BADNESS','BackgroundColor',[1 1 1],'color','r')
    text(1,-thresholdvalue+1,'FUZZY LOGIC LINE OF BADNESS','BackgroundColor',[1 1 1],'color','r')
    
%     title(['DC Offset Calibration data for ' foldernames{freqsv(N)}]);
    xlabel('Channel Number')
    ylabel('DC offset calibration value')
    hold off
    ...
    if isempty(bad_idx) ==1;
        ...
        disp(['There were no bad channels for frequency ' foldernames{freqsv(N)} ' HOORAY! :)']);
         title(['DC offset Calibration data for ' foldernames{freqsv(N)} ' \color[rgb]{0 0.5 0}EVERYTHING IS FINE! :)'],'FontWeight','bold');
    else
        badchnstr='';
        ...
        %moan about it
        disp(['BOO bad channels for frequency ' foldernames{freqsv(N)} ': ' badchnstr]);
        title(['DC offset Calibration data for ' foldernames{freqsv(N)} ' \color{red}Bad Channels: ' badchnstr ],'FontWeight','bold');
    end
    
    %range of values IGNORING BAD CHANNELS
...
DC_offset_data.cal_data=cal_data; %%%change this WOOOOPPPPP
DC_offset_data.bad_chns=bad_chns;
